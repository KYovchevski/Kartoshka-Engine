...
void krt::Application::CreateRenderPass()
{
    ...
    subpassDescription.pDepthStencilAttachment = nullptr; // Attachments for depth stencil buffers
    ...
    vkCreateRenderPass(m_LogicalDevice->GetVkDevice(), &info, nullptr, &rp);

    m_ForwardRenderPass = std::make_unique<RenderPass>(*m_ServiceLocator, createInfo);
    m_ForwardRenderPass->m_VkRenderPass = rp;
}
...
void krt::Application::DrawFrame()
{
    ...
    auto frameInfo = m_Window->GetNextFramebuffer(m_ImageAvailableSemaphore);
